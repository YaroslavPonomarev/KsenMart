...
class JFormFieldRegions extends JFormField {
	...
	public function getInput(){
		...
		$res_html.=KSSystem::wrapFormField($this->element,$html);
		...
		foreach($this->value as $country_id=>$regions)
		{
			...
			foreach($regions as $region_id)
				if (isset($countries[$country_id]) && isset($countries[$country_id]->regions[$region_id]))
					$html.='	<li region_id="'.$region_id.'"><span>'.$countries[$country_id]->regions[$region_id]->title.'</span><a href="#"></a><input type="hidden" name="'.$this->name.'['.$country_id.'][]" value="'.$region_id.'"></li>';
		}			
		...
		function addCountry(country_id)
		{
			...
			jQuery(".countries .no-countries").hide();
			jQuery(".countries ul").append(html);
		...
		{
			jQuery(".countries li[country_id="+country_id+"]").remove();
			if (jQuery(".countries li").length==1)
				jQuery(".countries .no-countries").show();
		...
		function addRegion(region_id)
		{
			...
			jQuery(".regions .no-regions").hide();
			jQuery(".regions ul").append(html);
		}
		
		function removeRegion(region_id)
		{
			jQuery(".regions li[region_id="+region_id+"]").remove();
			if (jQuery(".regions li").length==1)
				jQuery(".regions .no-regions").show();
