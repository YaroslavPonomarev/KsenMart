...
class JFormFieldCommentUser extends JFormField {
	...
	public function getInput(){
		...
		{
			$html.='<div class="row">';
			$html.='	<a href="'.JRoute::_('index.php?option=com_ksenmart&view=users&layout=search&items_tpl=commentuser&items_to=ksm-slidemodule-commentuser-container&tmpl=component').'" class="add">'.JText::_('ksm_add').'</a>';
			$html.='</div>';
		}
		$html.='</div>';
		$html=KSSystem::wrapFormField($this->element,$html);
		$script='
		jQuery(".ksm-slidemodule-commentuser .add").live("click", function(){
		...
		jQuery(".ksm-slidemodule-commentuser .del").live("click", function(){
