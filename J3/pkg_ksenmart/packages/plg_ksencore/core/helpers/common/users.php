...
class KSUsers {
    ...
    public static function getUser($id = '') {
        ...
        if ($id != 0) {
            ...
            $query = KSMedia::setItemMainImageToQuery($query, 'user', 'km_u.');
            ...
            $user->small_img = KSMedia::resizeImage($user->filename, $user->folder, $params->get('admin_product_thumb_image_width', 36) , $params->get('admin_product_thumb_image_heigth', 36) , json_decode($user->params, true));
            $user->medium_img = KSMedia::resizeImage($user->filename, $user->folder, $params->get('admin_product_medium_image_width', 120) , $params->get('admin_product_medium_image_heigth', 120) , json_decode($user->params, true));
    ...
    public static function getUserGroups($uid) {
        ...
        $query->select($db->quoteName(array(
            'group_id'
        )))->from($db->quoteName('#__user_usergroup_map'))->where($db->quoteName('user_id') . '=' . $db->getEscaped($uid));
    ...
    public static function getFields() {
        ...
        $query->select('
                uf.id,
                uf.title
            ')->from('#__ksenmart_user_fields AS uf')->order('uf.ordering');
        ...
        if (!empty($user) && is_object($user)) {
            ...
            if (!empty($fields)) {
                ...
                $query->select('
                        ...
                        ufv.value,
                        ufv.field_id
                    ')->from('#__ksenmart_user_fields_values AS ufv')->where('ufv.user_id=' . $db->getEscaped($user->id))->where('(ufv.field_id IN(' . implode(', ', $field_ids) . '))');
    ...
    public function removeUserSubscribeGroup($uid) {
        if (!empty($uid) && $uid > 0) {
            ...
            $conditions = array(
                'user_id=' . $db->getEscaped($uid) ,
                'group_id=' . $db->getEscaped(KSUsers::getSubscribersGroupID())
            );
