...
class KSSystem {
    ...
    public static function getModel($name, $config = array()) {
        ...
        if (file_exists($modelFile)) {
            ...
            JModel::addIncludePath(JPATH_SITE . DS . 'components' . DS . self::$ext_name_com . DS . 'models');
        }
    ...
    public static function getSeoTitlesConfig($part, $type = 'title') {
        if (!empty($part)) {
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select('config')->from('#__ksenmart_seo_config')->where('type="' . $type . '"')->where('part="' . $db->getEscaped($part) . '"');
            $db->setQuery($query);
    ...
    public static function loadTemplate(array $vars, $view = 'catalog', $layout = 'default', $tmpl = 'item') {
        ...
        if (!is_object(self::$_template_controller)) {
            if ($option != 'com_ksenmart') {
                self::$_template_controller = KSSystem::getController($view);
            } else {
