...
trackbar.hotSearch.prototype = {
	...
	*/
    ...
    init: function(hash, node) {
        ...
        try {
            ...
            var code = '<div class="tickdiv" id="' + this.TICKDIV_PREFIX + this.id + '"></div><table id="' + this.TABLE_PREFIX + this.id + '" ' + (this.width ? ' style="width:' + this.width + 'px;"' : '') + 'class="trackbar" onSelectStart="return false;">\
				<tr>\
					<td class="l"><div id="leftBlock_' + this.id + '"><span></span><span class="limit"></span><img id="leftBegun_' + this.id + '" ondragstart="return false;" src="' + URI_ROOT + '/components/com_ksenmart/css/i/b_l.png" width="12" height="12" alt="" /></div></td>\
					<td class="c" id="centerBlock_' + this.id + '"></td>\
					<td class="r"><div id="rightBlock_' + this.id + '"><span></span><span class="limit"></span><img id="rightBegun_' + this.id + '" ondragstart="return false;" src="' + URI_ROOT + '/components/com_ksenmart/css/i/b_r.png" width="12" height="12" alt="" /></div></td>\
				</tr>\
			</table>';
    ...
    drawTicks: function() {
        if (this.showBigTicks)
            for (var i = 1; i <= this.bigTicks; i++) this.tickDiv.innerHTML += '<div style="left:' + (this.widthRem + i * this.valueWidth / (this.bigTicks + 1)) + 'px" class="tick"><span>' + parseInt(this.leftLimit + i * this.valueInterval / (this.bigTicks + 1)) + '</span></div>';
        if (this.showSmallTicks)
            for (i = 1; i <= (this.smallTicks + 1) * (this.bigTicks + 1) - 1; i++) this.tickDiv.innerHTML += '<div style="left:' + (this.widthRem + i * this.valueWidth / ((this.smallTicks + 1) * (this.bigTicks + 1))) + 'px" class="stick"></div>';
    },
    ...
    onMove: function() {
        document.getElementById('search-price-less').value = (!this.dual && this.leftWidth > this.width / 2) ? "" : this.leftValue;
        document.getElementById('search-price-more').value = (!this.dual && this.rightWidth >= this.width / 2) ? "" : this.rightValue;
        KMChangeFilter(document.getElementById('search-button'), '');
    },
    moveHandler: function(evt) {
        ...
        this.setCurrentState();
        this.Timer = setTimeout('trackbarMove(' + this.dual + ',' + this.leftValue + ',' + this.leftWidth + ',' + this.rightValue + ',' + this.rightWidth + ',' + this.width + ')', 1000);
    },
...
function trackbarMove(dual, leftValue, leftWidth, rightValue, rightWidth, width) {
    document.getElementById('search-price-less').value = (!dual && leftWidth > width / 2) ? "" : leftValue;
    document.getElementById('search-price-more').value = (!dual && rightWidth >= width / 2) ? "" : rightValue;
    KMChangeFilter(document.getElementById('search-button'), '');
}