...
class KsenMartModelcatalog extends JModelKSList {
    ...
    public function getCategory() {
        ...
        if(!empty($this->_categories)){
            ...
            if(!empty($category)){
                $category->image = KSMedia::resizeImage($category->filename, $category->folder, 0, 32, json_decode($category->params, true));
            }
    ...
    public function setCatalogMetaData() {
        ...
        $categories     = implode(';', $this->_categories);
        $manufacturers  = implode(';', $this->_manufacturers);
        $properties     = implode(';', $this->_properties);
        $countries      = implode(';', $this->_countries);
        ...
        $query
            ->select('
                ...
                metakeywords
            ')
            ->from('#__ksenmart_seo_texts')
            ->where('categories="' . $categories . '"')
            ->where('manufacturers="' . $manufacturers . '"')
    ...
    public function setLayoutCatalog($layout) {
        ...
        if (!empty($layout)) {
            ...
            try {
                $result = $this->_db->updateObject('#__ksenmart_users', $user_update, 'id');
