        ...
        foreach ($config as $key => $val) {
            if ($val->user == 0) {
                if ($val->active == 1) {
                    ...
                    if ($key == 'seo-category' || $key == 'seo-parent-category') {
                        $final_categories = array();
                        $query = $this->_db->getQuery(true);
                        $query
                            ->select('category_id')
                        ...
                        ;
                        $this->_db->setQuery($query);
                        $product_categories = $this->_db->loadObjectList();
                        foreach ($product_categories as $product_category) {
                            if (!empty($default_category)){
                                $id_default_way = false;
                            } else {
                            ...
                            while ($parent != 0) {
                                ...
                                $category = KSSystem::getTableByIds(array($parent), 'categories', array('t.title', 't.parent_id'), true, false, true);
                        ...
                        if ($key == 'seo-parent-category'){
                            for ($k = 0; $k < count($final_categories) - 1; $k++){
                                $title[] = $final_categories[$k];
                            }
                        }elseif (count($final_categories) > 0){
                            $title[] = $final_categories[count($final_categories) - 1];
                        }
                    }
                }
            } else {
