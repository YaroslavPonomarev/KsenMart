    ...
    public function getProductComments($pid) {
        ...
        if (!empty($pid) && $pid > 0) {
            ...
            $query->select('
                    ...
                    uf.filename AS logo, 
                    u.name
                ')->from('#__ksenmart_comments AS c')->leftjoin('#__ksenmart_users AS kmu ON kmu.id=c.user_id')->leftjoin('#__users AS u ON kmu.id=u.id')->leftjoin('#__ksenmart_files AS uf ON uf.owner_id=u.id')->where('c.product_id=' . $this->_db->escape($pid))->where('c.published=1')->where('c.parent_id=0')->order('c.date_add DESC');
    ...
    public function getProductCommentsChild($comments) {
        ...
        if (!empty($comments)) {
            ...
            $query->select('
                    ...
                    uf.filename AS logo, 
                    u.name
                ')->from('#__ksenmart_comments AS c')->leftjoin('#__ksenmart_users AS kmu ON kmu.id=c.user_id')->leftjoin('#__users AS u ON kmu.id=u.id')->leftjoin('#__ksenmart_files AS uf ON uf.owner_id=u.id')->where('c.published=1');
