		...
		if (isset($_REQUEST['state']) and isset($_SESSION['state'])) {
			...
			if ($_REQUEST['state'] == $_SESSION['state']) {
				...
				if (count($db_user) == 0) {
					...
					$query = "insert into #__ksenmart_users (`id`) values ('$user->id')";
					$db->setQuery($query);
					$db->Query();
				} else {
					$app = JFactory::getApplication();
...
?>
	...
	function add_favorites() {
		...
		if ($user->id != 0 && !in_array($id, $user->favorites)) {
			$user->favorites[] = $id;
			$db = JFactory::getDBO();
			echo $query = "update #__ksenmart_users set favorites='" . json_encode($user->favorites) . "' where id='$user->id'";
			$db->setQuery($query);
			$db->query();
		}
		exit();
	...
	function add_watched() {
		...
		if ($user->id != 0 && !in_array($id, $user->watched)) {
			$user->watched[] = $id;
			$db = JFactory::getDBO();
			$query = "update #__ksenmart_users set watched='" . json_encode($user->watched) . "' where id='$user->id'";
			$db->setQuery($query);
			$db->query();
		}
		exit();
	...
	function subscribe() {
		...
		if ($user->id != 0 && !in_array(2, $user->groups)) {
			...
			$user->groups[] = 2;
			$db = JFactory::getDBO();
			$query = "update #__ksenmart_users set groups='|" . implode('||', $user->groups) . "|' where id='$user->id'";
			$db->setQuery($query);
			$db->query();
		}
		exit();
