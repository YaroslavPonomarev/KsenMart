		...
		<thead>
			<tr>
				<th align="left" style="position:relative;">
					<?php echo JText::_('ksm_seo-meta-config-'.$config_key)?>
			...
			<td class="rightcol" style="background:#f9f9f9!important;padding-top:15px;">
				<div class="row">
					...
					<select name="config[<?php echo $config_key;?>][description][flag]" id="sel_<?php echo $config_key;?>_dflag" class="sel">
						<option value="1" <?php echo ($config->config->description->flag==1?'selected':'');?>><?php echo JText::_('ksm_seo-meta-make-from');?></option>
						<option value="0" <?php echo ($config->config->description->flag==0?'selected':'');?>><?php echo JText::_('ksm_seo-meta-not-make');?></option>
					</select>	
					<select name="config[<?php echo $config_key;?>][description][type]" id="sel_<?php echo $config_key;?>_dtype" class="sel marginleft20">
						...
						<option value="<?php echo $type;?>" <?php echo ($config->config->description->type==$type?'selected':'');?>><?php echo JText::_('ksm_'.$type);?></option>
						<?php endforeach;?>
					</select>
					<div class="seo-meta-symbols">
						<?php echo JText::_('ksm_seo-meta-not-more');?>
						...
						<?php echo JText::_('ksm_seo-meta-symbols');?>
					</div>
				</div>
				<div class="row">
					...
					<select name="config[<?php echo $config_key;?>][keywords][flag]" id="sel_<?php echo $config_key;?>_kflag" class="sel">
						<option value="1" <?php echo ($config->config->keywords->flag==1?'selected':'');?>><?php echo JText::_('ksm_seo-meta-make-from');?></option>
						<option value="0" <?php echo ($config->config->keywords->flag==0?'selected':'');?>><?php echo JText::_('ksm_seo-meta-not-make');?></option>
					</select>	
					<select name="config[<?php echo $config_key;?>][keywords][type]" id="sel_<?php echo $config_key;?>_ktype" class="sel marginleft20">
						...
						<option value="<?php echo $type;?>" <?php echo ($config->config->keywords->type==$type?'selected':'');?>><?php echo JText::_('ksm_'.$type);?></option>
						<?php endforeach;?>
					</select>
				</div>	
			</td>
