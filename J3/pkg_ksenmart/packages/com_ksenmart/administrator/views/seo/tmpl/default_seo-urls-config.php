			...
			<td class="rightcol" style="background:#f9f9f9!important;padding-top:15px;">
				<div class="row">
					<div class="seo-blocks">
						<div class="seo-block seo-block-first" alt="<?php echo JText::_('ksm_seo-first-urls-example')?>">
							<label class="seo-block-content">...</label>
						</div>
						<?php foreach($config->config as $key=>$block_config):?>
						<div class="seo-block <?php echo ($block_config->activable?'seo-block-activable':'')?> <?php echo ($block_config->sortable?'seo-block-sortable':'')?> <?php echo (!$block_config->active?'inactive':'')?>" alt="<?php echo ($block_config->user==1?'/'.$block_config->title:JText::_('ksm_'.$key.'-urls-example'));?>">
						...
						<div class="seo-block-bottom">
							...
							<a class="add" href="<?php echo JRoute::_('index.php?option=com_ksenmart&view=seo&layout=seourlvalue&tmpl=component');?>"><?php echo JText::_('ksm_add')?></a>
						</div>	
					</div>
				</div>
			</td>
