            ...
            if($k > 0) {
                ...
                foreach($categories as $cats) {
                    ...
                    foreach($cats as $cat) {
                        ...
                        if($cat != '') {
                            $query = $this->_db->getQuery(true);
                            $query->select('*')->from('#__ksenmart_categories')->where('title=' . $this->_db->quote($cat))->where('parent=' . $parent);
                            $this->_db->setQuery($query);
    ...
    function delPhoto($filename, $folder) {
        ...
        foreach($files as $file) {
            if($file != '.' && $file != '..' && is_dir(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $file))
                if(file_exists(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $file . '/' . $filename)) unlink(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $file . '/' . $filename);
        }
        $query = $this->_db->getQuery(true);
