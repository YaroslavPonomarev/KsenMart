...
class KsenMartModelSeo extends JModelKSAdmin {
	...
	protected function populateState($ordering = null, $direction = null){
		...
		$seo_type=$app->getUserStateFromRequest('com_ksenmart.seo.seo_type', 'seo_type','text');
	...
	function saveUrlsConfigs($config) {
		...
		foreach ($config as $key => $val) {
			$query = $this->_db->getQuery(true);
			$query->update('#__ksenmart_seo_config')->set('config=' . $this->_db->quote(json_encode($val)))->where('part=' . $this->_db->quote($key))->where('type="url"');
			$this->_db->setQuery($query);
			$this->_db->Query();
		}
	...
	function saveMetaConfigs($config) {
		...
		foreach ($config as $key => $val) {
			$query = $this->_db->getQuery(true);
			$query->select('config')->from('#__ksenmart_seo_config')->where('part=' . $this->_db->quote($key))->where('type="meta"');
			$this->_db->setQuery($query);
			...
			$query = $this->_db->getQuery(true);
			$query->update('#__ksenmart_seo_config')->set('config=' . $this->_db->quote(json_encode($val)))->where('part=' . $this->_db->quote($key))->where('type="meta"');;
			$this->_db->setQuery($query);
			$this->_db->Query();
		}
	...
	function saveTitlesConfigs($config) {
		...
		foreach ($config as $key => $val) {
			$query = $this->_db->getQuery(true);
			$query->update('#__ksenmart_seo_config')->set('config=' . $this->_db->quote(json_encode($val)))->where('part=' . $this->_db->quote($key))->where('type="title"');
			$this->_db->setQuery($query);
			$this->_db->Query();
		}
	...
	function getListItems() {
		...
		foreach ($seo_texts as & $seo_text) {
			...
			foreach ($seo_text->manufacturers as & $manufacturer) {
				...
				$this->_db->setQuery($query);
				$title = $this->_db->loadResult();
				if (!empty($title)) $manufacturer = $title;
			}
			$properties = array();
			...
			foreach ($seo_text->properties as $property) {
				...
				$property_value = $this->_db->loadObject();
				if (!empty($property_value)) {
					if (!isset($properties[$property_value->property_id])) $properties[$property_value->property_id] = array();
					if (!empty($property_value->title)) $properties[$property_value->property_id][] = $property_value->title;
				}
			}
			...
			foreach ($properties as $key => $property_values) {
				...
				$this->_db->setQuery($sql);
				$title = $this->_db->loadResult();
				if (!empty($title)) $seo_text->properties[] = $title . '=' . implode('+', $property_values);
			}
