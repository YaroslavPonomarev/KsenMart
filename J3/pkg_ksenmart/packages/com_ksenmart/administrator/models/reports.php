...
class KsenMartModelReports extends JModelKSAdmin {
	...
	function getWatchedProducts()
	{
		...
		if ($order_type!='watched')
			$order_type='p.'.$order_type;	
		$query=$this->_db->getQuery(true);
		$query->select('*')->from('#__ksenmart_users');
		$this->_db->setQuery($query);
		...
		$query=$this->_db->getQuery(true);		
		$query->select('SQL_CALC_FOUND_ROWS p.*,(select count(id) from #__ksenmart_users where INSTR(watched,p.id)!=0) as watched')->from('#__ksenmart_products as p')->order($order_type.' '.$order_dir);
	...
	function getFavoritesProducts()
	{
		...
		$query=$this->_db->getQuery(true);		
		$query->select('SQL_CALC_FOUND_ROWS p.*,(select count(id) from #__ksenmart_users where INSTR(favorites,p.id)!=0) as favorites')->from('#__ksenmart_products as p')->order($order_type.' '.$order_dir);
