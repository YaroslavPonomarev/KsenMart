...
class KsenMartModelShippings extends JModelKSAdmin {
	...
	function getListItems()
	{
		...
		->leftjoin('#__extensions as e on e.element=s.type and e.folder="KSMShipping"')
	...
	function getShippingParamsForm()
	{
		...
		$query=$this->_db->getQuery(true);
		$query->select('enabled')->from('#__extensions')->where('element='.$this->_db->quote($type))->where('folder="KSMShipping"');
		$this->_db->setQuery($query);
