...
class com_ksenmartInstallerScript {
	...
	function postflight($type, $parent) {
		...
		self::KMinstallModule('mod_km_settings_groups', 'mod_km_settings_groups', 1, 'km-list-left', '{"views":["settings"],"layout":"_:default","moduleclass_sfx":"","cache":"0"}', 1);
		...
		JFolder::create(JPATH_ROOT . DS . 'plugins' . DS . 'kmdiscount');
		JFolder::create(JPATH_ROOT . DS . 'plugins' . DS . 'KSMShipping');
		...
		self::KMinstallPlugin('couriermoscow', 'KSMShipping', 'KSM_SHIPPING_COURIERMOSCOW', 1, '{}');
		self::KMinstallPlugin('fixedregions', 'KSMShipping', 'KSM_SHIPPING_FIXEDREGIONS', 1, '{}');
	...
	private static function KMinstallModule($title, $module, $ordering, $position = 'position-4', $params = '{}', $client_id = 0, $published = 1) {
		$path = JPATH_ROOT . DS . 'administrator' . DS . 'components' . DS . 'com_ksenmart' . DS . 'install';
		$db = JFactory::getDBO();
		$access = 1;
		...
		$query = $db->getQuery(true);
		$query->insert('#__modules')->columns('title,module,ordering,position,client_id,published,access,params')->values(implode(',', $values));
		$db->setQuery($query);
		if (!$db->query()) {
			$app = JFactory::getApplication();
		...
		if (version_compare(JVERSION, '1.6.0', 'ge')) {
			...
			if (empty($ext_id)) {
				...
				$values = array(
					...
					$access,
					0,
					$db->quote($manifest_cache) ,
					$db->quote($params) ,
					$ordering
				);
				$query = $db->getQuery(true);
				$query->insert('#__extensions')->columns('name,type,element,folder,client_id,enabled,access,protected,manifest_cache,params,ordering')->values(implode(',', $values));
				$db->setQuery($query);
				if (!$db->query()) {
					$app = JFactory::getApplication();
					$app->enqueueMessage($db->getErrorMsg());
	...
	private static function KMinstallPlugin($plugin, $folder, $title, $ordering, $params) {
		$path = JPATH_ROOT . DS . 'administrator' . DS . 'components' . DS . 'com_ksenmart' . DS . 'install';
		$db = JFactory::getDBO();
		$access = 1;
