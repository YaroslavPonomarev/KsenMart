...
class KSMOrders {
        ...
        if(!empty($oid)) {
            ...
            if(!empty($order)) {
                ...
                foreach($order as $o_item) {
                    ...
                    $o_item->val_price = KSMPrice::showPriceWithTransform($o_item->price);
                }
            }
    ...
    public static function getPrintformCustomerName($customer_fields) {
        $customer_name = '';
        if(isset($customer_fields['lastname']) && !empty($customer_fields['lastname'])) $customer_name .= $customer_fields['lastname'] . ' ';
        if(isset($customer_fields['name']) && !empty($customer_fields['name'])) $customer_name .= $customer_fields['name'] . ' ';
        if(isset($customer_fields['surname']) && !empty($customer_fields['surname'])) $customer_name .= $customer_fields['surname'];
