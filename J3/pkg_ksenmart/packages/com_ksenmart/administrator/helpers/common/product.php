...
class KSMProducts {
    ...
    public static function getProduct($id) {
        ...
        if($row && !empty($row)) {
            ...
            $row->price = KSMPrice::getPriceInDefaultCurrency($row->price, $row->price_type);
            $row->val_price_wou = KSMPrice::getPriceWithDiscount($row->price, 2);
            if($row->val_price_wou < 0) {
                $row->val_price_wou = 0;
            }
            $row->val_price = KSMPrice::showPriceWithTransform($row->val_price_wou);
            $row->old_price = KSMPrice::getPriceInDefaultCurrency($row->old_price, $row->price_type);
            $row->val_old_price_wou = KSMPrice::getPriceWithDiscount($row->old_price, 2);

            if($row->val_old_price_wou < 0) {
                $row->val_old_price_wou = 0;
            }
            $row->val_old_price      = KSMPrice::showPriceWithTransform($row->val_old_price_wou);
            $row->val_diff_price_wou = $row->val_old_price_wou - $row->val_price_wou;
            $row->val_diff_price     = KSMPrice::showPriceWithTransform($row->val_diff_price_wou);
    ...
    public static function getPriceWithProperties($product_id, $properties = array(), $price = null) {
        ...
        if(empty($price)) {
            ...
            $price = KSMPrice::getPriceInDefaultCurrency($product->price, $product->price_type);
        }
