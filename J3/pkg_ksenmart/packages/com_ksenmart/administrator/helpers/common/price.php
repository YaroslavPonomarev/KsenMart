...
class KSMPrice {
    ...
    public static function showPriceWithoutTransform($price, $currency = 0) {
        if (empty(self::$_currencies)) {
            ...
            foreach ($rows as $v) {
                self::$_currencies[$v->id] = $v;
                if ($v->default == 1) {
                    self::$_currencies[0] = $v;
                }
            }
        }
    ...
    public static function getPriceInDefaultCurrency($price, $currency) {
        if (empty(self::$_currencies)) {
            $db = JFactory::getDBO();
            $query = "select * from #__ksenmart_currencies";
            $db->setQuery($query);
    ...
    public static function getPriceInCurrentCurrency($price, $currency) {
        ...
        $curr_currency = $app->getUserState('com_ksenmart.curr_currency', 0);
        if (empty(self::$_currencies)) {
            $db = JFactory::getDBO();
            $query = "select * from #__ksenmart_currencies";
            $db->setQuery($query);
