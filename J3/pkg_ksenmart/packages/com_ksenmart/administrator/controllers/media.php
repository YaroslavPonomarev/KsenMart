...
if (!class_exists('SimpleImage')){
	require JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'additional'.DS.'simpleimage.php';
}
jimport('joomla.filesystem.file');
jimport('joomla.filesystem.folder');

class KsenMartControllerMedia extends KsenMartController
{
	...
	function upload_image() {
		...
		$url = KSMedia::resizeImage(basename($fileName),$folder);
    ...
    function delete_photo() {
        ...
        KSMedia::deletePhoto($filename, $folder);
        JFactory::getApplication()->close();
    }	
	...
	function upload_file() {
		...
		echo json_encode(array('error'=>'', 'filename'=>str_replace(JPATH_ROOT, '', $fileName), 'url'=>$url, 'title'=>$pathinfo['filename']));
		JFactory::getApplication()->close();
