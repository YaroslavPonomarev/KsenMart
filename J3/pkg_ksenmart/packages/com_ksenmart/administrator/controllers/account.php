...
class KsenMartControllerAccount extends JControllerLegacy {
    ...
    public function __construct(){
        ...
        $this->_model   = KSSystem::getModel('account');
    ...
    public function getAccountInfo(){
        if($this->checkAuthorize()){
            $this->_account_info                    = new stdClass();
    ...
    public function createDomainContact(){
        ...
        $params['firstname']    = ucfirst(KSFunctions::GenAlias($jinput->post->get('firstname_ru', null, 'string')));
        $params['lastname']     = ucfirst(KSFunctions::GenAlias($jinput->post->get('lastname_ru', null, 'string')));
        $params['middlename']   = ucfirst(substr(KSFunctions::GenAlias($jinput->post->get('middlename_ru', null, 'string')), 0, 1));
    ...
    public function setReg(){
        ...
        if(!empty($_data)){
            if($_data->passwd == $_data->confirm){
                ...
                $response = json_decode(KSSystem::getLdmApiData('register', null, $_data));
    ...
    public function testAPI(){
        ...
        echo KSSystem::getLdmApiData('register', null, $params);
    }
}