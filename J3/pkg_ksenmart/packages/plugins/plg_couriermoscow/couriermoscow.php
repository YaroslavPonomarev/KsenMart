...
if (!class_exists('KSMShippingPlugin')) {
	require (JPATH_ROOT . DIRECTORY_SEPARATOR . 'administrator' . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_ksenmart' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'kmshippingplugin.php');
}
...
class plgKMShippingCourierMoscow extends KSMShippingPlugin {
	...
	function onAfterExecuteKSMCartGetcart($model, $cart = null) {
		...
		$cart->shipping_sum_val = KMPrice::showPriceWithTransform($cart->shipping_sum);
		$cart->total_sum+= $cart->shipping_sum;
		$cart->total_sum_val = KMPrice::showPriceWithTransform($cart->total_sum);
	...
	public function onAfterExecuteKSMOrdersGetorder($model, $order = null) {
		...
		$order->costs['shipping_cost_val'] = KMPrice::showPriceWithTransform($order->costs['shipping_cost']);
