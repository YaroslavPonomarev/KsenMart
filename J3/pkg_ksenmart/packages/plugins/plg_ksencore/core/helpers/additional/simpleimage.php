...
class SimpleImage {
	...
	//
	public function overlay($overlay_file, $position = 'middle center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
	...
	function background($width=0,$height=0,$params=array(),$file=null,$color=null)
	{
		...
		{
			case 'center':$dst_x=round($width/2-$this->width/2);break;
			case 'right':$dst_x=round($width-$this->width);break;
		}
	...
	public function watermark($watermark_file,$params=array()) {
		...
		switch($params['watermark_type'])
		{
			case '0':
				...
				$x=0;
				$y=0;
				switch ($params['watermark_halign']) {
					case 'center':
						$x = round($this->width / 2 - $watermark->width / 2);
						break;
					case 'right':
						$x = round($this->width - $watermark->width);
						break;
				}
				switch ($params['watermark_valign']) {
					case 'middle':
						$y = round($this->height / 2 - $watermark->height / 2);
						break;
					case 'bottom':
			...
			case '2':
				...
				$diff_y=floor(($this->height - $watermark->height*$count_y)/($count_y+1));
				for($k=0;$k<$count_x;$k++)
				{
					for($i=0;$i<$count_y;$i++)
					{
