...
class KSMedia {
    ...
    public static function resizeImage($file, $folder, $width = 120, $height = 120, $iparams = array()) {
    ...
    public static function saveItemMedia($id = null, $data = array(), $owner_type = null, $folder = null) {
        ...
        $query = $db->getQuery(true);
        $query->delete('#__'. $ext_name .'_files')->where('owner_id=' . $owner_id)->where('owner_type=' . $db->quote($owner_type));
        if(count($in)) {
    ...
    public static function setItemMainImageToQuery($query, $owner_type = 'product', $l = 'p.') {
        global $ext_name, $ext_name_com;
        
        $query->select('(select f.filename from #__'. $ext_name .'_files as f where f.owner_id=' . $l . 'id and owner_type="' . $owner_type . '" and media_type="image" order by ordering limit 1) as filename');
        $query->select('(select f.folder from #__'. $ext_name .'_files as f where f.owner_id=' . $l . 'id and owner_type="' . $owner_type . '" and media_type="image" order by ordering limit 1) as folder');
        $query->select('(select f.params from #__'. $ext_name .'_files as f where f.owner_id=' . $l . 'id and owner_type="' . $owner_type . '" and media_type="image" order by ordering limit 1) as params');
        return $query;
    }
}