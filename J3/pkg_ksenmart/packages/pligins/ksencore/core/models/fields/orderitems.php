...
class JFormFieldOrderItems extends JFormField {
	...
	public function getInput() {
		...
		$script = '
		jQuery(".order-positions .add").on("click",function(){
		...
		jQuery(".order-positions .quants .minus").on("click",function(){
		...
		jQuery(".order-positions .quants .plus").on("click",function(){
			var item=jQuery(this).parents(".position:first");
			var	count=parseFloat(item.find(".quants .inputbox").val());
			var product_packaging=parseFloat(item.find(".product_packaging").val());
			count = Math.ceil(count/product_packaging)*product_packaging;
		...
		jQuery(".order-positions .quants .inputbox").on("keypress",function(e){
			if (e.keyCode==13)
			{	
				var item=jQuery(this).parents(".position:first");
				var	count=parseFloat(item.find(".quants .inputbox").val());
		...
		jQuery(".order-positions .del").on("click",function(){
		...
		jQuery(".order-positions .properties input").on("change",function(){
			onChangeItems();	
		});		
