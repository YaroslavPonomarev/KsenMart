...
class JFormFieldShippingAddressFields extends JFormField {
	
	protected $type = 'ShippingAddressFields';
	...
	public function getInput() {
		...
		$html.= '	</div>';
		$html.= '</div>';
		$html.= '<div id="popup-window-address-fields" class="popup-window">';
		...
		$html.= '							<select class="sel" id="new-address-field-type" style="width:113px;">';
		...
		jQuery(document).ready(function(){
			jQuery(".shipping-address-fields .fields-row .cb-enable").on("click",function(){
			...
			jQuery(".shipping-address-fields .fields-row .cb-disable").on("click",function(){
				jQuery(this).addClass("selected");
				jQuery(this).parents(".row:first").find(".cb-enable").removeClass("selected");			
				jQuery(this).parents(".row:first").find(".published").val("0");
			});				
			jQuery(".shipping-address-fields .fields-row .del").on("click",function(){
				jQuery(this).parents(".row:first").remove();
			});
			jQuery(".shipping-address-fields .add").on("click",function(){
				jQuery("#popup-window-address-fields").show();
				return false;
			});
			jQuery("#new-address-field-type").on("change",function(){
				if (jQuery(this).val()=="text")
				{
					jQuery("#popup-window-address-fields .add-row").hide();
					jQuery("#popup-window-address-fields .values-row").hide();
				}
				else
				{
					jQuery("#popup-window-address-fields .add-row").show();
					jQuery("#popup-window-address-fields .values-row").show();				
				}
			});	
			jQuery("#popup-window-address-fields .add").on("click",function(){
				var count=jQuery("#popup-window-address-fields .values-row .row").length;
				count++;
				var html="";
				html+="<div class=\'row\'>";
				...
				html+="</div>";
				jQuery("#popup-window-address-fields .values-row").append(html);
				return false;
			});
			jQuery("#popup-window-address-fields .saves").on("click",function(){
				var title=jQuery("#popup-window-address-fields .new-field-name").val();
				if (title=="")
					return false;
				var type=jQuery("#new-address-field-type").val();
				var ordering=jQuery(".shipping-address-fields .fields-row .row").length;
				ordering++;
				var key=ordering*(-1);
				var html="";
				html+="<div class=\'row\'>";
				...
				if (type=="select")
				{
					html+="		<select class=\'sel\' id=\'new-address-field-"+ordering+"\' style=\'width:113px;\'>";		
					jQuery("#popup-window-address-fields .values-row .row .inputbox").each(function(){
				...
				html+="				<span>' . JText::_('ksm_shippings_shipping_required_field') . '</span>";		
				html+="			</div>";		
				html+="			<input type=\'hidden\' name=\'' . $this->name . '["+key+"][position]\' value=\'address\' >";		
				...
				html+="		<a class=\'del\'></a>";			
				html+="</div>";	
				jQuery(".shipping-address-fields .fields-row").append(html);
				jQuery("#popup-window-address-fields .new-field-name").val("");
				jQuery("#popup-window-address-fields .values-row").html("");
				jQuery("#popup-window-address-fields").hide();
