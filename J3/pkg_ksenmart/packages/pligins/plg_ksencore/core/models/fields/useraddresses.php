...
class JFormFieldUserAddresses extends JFormField {
	...
	public function getInput() {
		...
		foreach($this->value as $value)
		{
			...
			$html.='		<div class="row">';
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_city').'</label>';
			$html.='			<input class="inputbox width150px" name="'.$this->name.'['.$value->id.'][city]" value="'.$value->city.'">';
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_zip').'</label>';
			$html.='			<input class="inputbox" name="'.$this->name.'['.$value->id.'][zip]" value="'.$value->zip.'">';	
			$html.='		</div>';
			$html.='		<div class="row" style="margin-bottom:0px;">';
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_street').'</label>';
			$html.='			<input class="inputbox width150px" name="'.$this->name.'['.$value->id.'][street]" value="'.$value->street.'">';
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_house').'</label>';
			$html.='			<input class="inputbox" name="'.$this->name.'['.$value->id.'][house]" value="'.$value->house.'">';	
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_floor').'</label>';
			$html.='			<input class="inputbox" name="'.$this->name.'['.$value->id.'][floor]" value="'.$value->floor.'">';	
			$html.='			<label class="inputname">'.JText::_('ksm_users_user_address_flat').'</label>';
		...
		$html.='			<h3>'.JText::_('ksm_users_user_empty_addresses').'</h3>';		
		$html.='		</div>';		
		$html.='	</div>';
		$html.='	<div class="row">';
		$html.='		<a class="add">'.JText::_('ksm_add').'</a>';
		$html.='	</div>';
		$html.='</div>';
		
		$script='
		jQuery(".ksm-slidemodule-useraddresses .add").live("click",function(){
			var html="";
			...
			html+="			<label class=\"inputname\">'.JText::_('ksm_users_user_address_zip'). '</label>";
			html+="			<input type=\"text\" name=\"'.$this->name.'[-"+count+"][zip]\" class=\"inputbox\">";
			html+="		</div>";
			html+="		<div class=\"row\">";
			html+="			<label class=\"inputname\">'.JText::_('ksm_users_user_address_street'). '</label>";
		...
		jQuery(".ksm-slidemodule-useraddresses .del").live("click",function(){
