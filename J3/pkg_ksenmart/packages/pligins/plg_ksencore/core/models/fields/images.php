...
class JFormFieldImages extends JFormField {
    ...
    public function getInput() {
        ...
        $document->addScript(JURI::base() . 'components/com_ksenmart/js/swfupload/swfupload.js');
        $document->addScript(JURI::base() . 'components/com_ksenmart/js/swfupload/swfupload.queue.js');
        $document->addScript(JURI::base() . 'components/com_ksenmart/js/swfupload/js/fileprogress.js');
        $document->addScript(JURI::base() . 'components/com_ksenmart/js/swfupload/js/handlers.js');
        $document->addScript(JURI::base() . 'components/com_ksenmart/js/swfupload/config_image.js');
        $html = '';
        ...
        foreach($this->value as $file) {
            ...
            $source_filename = JPATH_ROOT . DS . 'media' . DS . 'ksengallery' . DS . $file->media_type . 's' . DS . $file->folder . DS . 'original' . DS . $file->filename;
            ...
            $thumb_url = KSMedia::resizeImage($source_filename, $file->folder, $cparams->get('admin_product_medium_image_width', 120), $cparams->get('admin_product_medium_image_height', 120), $params);
